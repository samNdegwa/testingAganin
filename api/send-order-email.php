...
$amount_due= mysqli_real_escape_string($mysqli, trim($request->amount_due));
$orderId = mysqli_real_escape_string($mysqli, trim($request->orderId));
...
try {
    ...
    $mail->Subject = 'Ekasline Order Confirmation';
    $mail->Body    = 'Dear Customer, your order number '.$orderId.' has been received successifully.<br> Order Number: '.$orderId.' <br> Total Item: '.$product_quantity.' <br> Total Cost: '.$amount_due.' <br> Pay this order through Mpesa paybill number 000000 and account number '.$orderId.'<br> 
        Thanks<br>
        <b>contact:</b><br>
        Ekasline <br>
